...
var onFeedback = function (deviceInfos) {
    ...
    console.error(new Date().toString() + " : " + JSON.stringify(deviceInfos));

    if(deviceInfos.length > 0){
        pushAssociations.removeDevices(deviceInfos.map(function (deviceInfo) {
            try {
                console.error(new Date().toString() + " : " + JSON.stringify(deviceInfo));
                return deviceInfo.device.token.toString("hex").toUpperCase();
            } catch (err) {
